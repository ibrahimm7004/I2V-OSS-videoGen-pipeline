...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a single Hunyuan I2V adapter smoke clip.")
    parser.add_argument("--input-image", type=Path, default=Path("assets/idea02/ref_01.png"))
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/_adapter_smoke/hunyuan"))
    parser.add_argument("--duration-sec", type=float, default=2.0)
    parser.add_argument("--steps", type=int, default=8)
    parser.add_argument("--fps", type=int, default=8)
    parser.add_argument("--seed", type=int, default=234567)
    ...
    config = load_config()
    adapter = get_adapter("hunyuan_i2v", "HUNYUAN_SMOKE", config)
    request = ClipRequest(
        clip_index=0,
        prompt="A clean cafe desk scene with a focused person working quietly, realistic handheld style.",
        negative_prompt="text, watermark, logo, blurry, low quality, deformed anatomy",
    ...
    print(f"Hunyuan smoke clip: {result.output_video_path}")
