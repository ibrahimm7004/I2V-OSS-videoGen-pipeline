...
class HunyuanI2VAdapter(ModelAdapter):
    """
    HunyuanVideo I2V adapter using diffusers HunyuanVideoImageToVideoPipeline.
    """
    ...
    def _import_runtime(self) -> tuple[Any, Any, Any]:
        try:
            ...
            raise RuntimeError("Hunyuan adapter requires torch. Install torch in this environment.") from exc

        try:
            from diffusers import HunyuanVideoImageToVideoPipeline  # type: ignore
            from diffusers.utils import export_to_video  # type: ignore
        except ImportError as exc:
            raise RuntimeError(
                "Hunyuan adapter requires diffusers with Hunyuan I2V support. "
                "Install/upgrade diffusers, transformers, accelerate, and safetensors."
            ) from exc

        return torch, HunyuanVideoImageToVideoPipeline, export_to_video

    def _resolve_repo_candidates(self) -> list[str]:
        env_repo = os.getenv("HUNYUAN_REPO_ID")
        candidates: list[str] = []
        if env_repo:
            candidates.append(env_repo.strip())

        for key in [self.model_id, "hunyuan_i2v"]:
            try:
        ...
        # Community repo may contain diffusers-formatted weights if official layout changes.
        if "hunyuanvideo-community/HunyuanVideo-I2V" not in candidates:
            candidates.append("hunyuanvideo-community/HunyuanVideo-I2V")
        if not candidates:
            candidates.append("tencent/HunyuanVideo-I2V")
        return candidates
    ...
    def _resolve_dtype_and_device(self, torch: Any) -> tuple[Any, str, str]:
        ...
        dtype_env = (os.getenv("HUNYUAN_DTYPE") or "bf16").strip().lower()
        if dtype_env == "fp16":
            dtype = torch.float16
            dtype_name = "float16"
        elif dtype_env == "fp32":
            ...
            raise ValueError("Invalid HUNYUAN_DTYPE. Use one of: fp16, bf16, fp32.")
    ...
    def _apply_memory_opts(self, pipe: Any, device: str, torch: Any) -> None:
        enable_offload = _as_bool(os.getenv("HUNYUAN_ENABLE_CPU_OFFLOAD"), default=False)
        if device == "cuda":
        ...
        if hasattr(pipe, "enable_xformers_memory_efficient_attention") and _as_bool(
            os.getenv("HUNYUAN_ENABLE_XFORMERS"), default=True
        ):
            try:
    ...
    def _ensure_pipeline(self) -> Any:
        ...
        torch, HunyuanVideoImageToVideoPipeline, export_to_video = self._import_runtime()
        self._torch = torch
        ...
        for repo_id in self._resolve_repo_candidates():
            ...
            try:
                pipe = HunyuanVideoImageToVideoPipeline.from_pretrained(repo_id, **kwargs)
        ...
        raise RuntimeError(
            "Failed to load Hunyuan I2V pipeline from available repos. "
            + " | ".join(load_errors)
        )
    ...
    def _apply_sampler_if_possible(self, pipe: Any, sampler_name: str | None) -> str | None:
        ...
        if normalized in {"ddim"}:
            ...
            LOGGER.warning("Sampler '%s' not mapped for Hunyuan; using pipeline default.", sampler_name)
            return None
    ...
    def _timeout_callback(self, started: float, max_runtime_seconds: int):
        def _callback(_: Any, __: int, ___: Any, callback_kwargs: dict[str, Any] | None) -> dict[str, Any]:
            if time.monotonic() - started > max_runtime_seconds:
                raise TimeoutError(f"Hunyuan adapter exceeded clip timeout ({max_runtime_seconds}s)")
    ...
    def _load_image(self, input_image: Path, width: int, height: int) -> Any:
        try:
            ...
            raise RuntimeError("Hunyuan adapter requires Pillow for conditioning image loading.") from exc

        if not input_image.exists():
            raise FileNotFoundError(f"Input image not found for Hunyuan adapter: {input_image}")
    ...
    def _extract_video_frames(self, output: Any) -> list[Any]:
        ...
        if not isinstance(frames, list) or not frames:
            raise RuntimeError("Hunyuan pipeline produced an empty frame list.")
        return frames

    def generate_clip(self, request: ClipRequest) -> ClipResult:
        if request.input_image is None:
            raise ValueError(
                "Hunyuan I2V requires an input image for every clip. "
        ...
        if torch is None or export_to_video is None:
            raise RuntimeError("Hunyuan adapter runtime was not initialized.")
        ...
        if time.monotonic() - started > request.max_runtime_seconds:
            raise TimeoutError(f"Hunyuan adapter exceeded clip timeout ({request.max_runtime_seconds}s)")
